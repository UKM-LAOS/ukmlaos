    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-20">
            ...
            <a href="{{ route('course.index') }}" class="flex items-center space-x-3">
                <img src="{{ asset('laos-course/front/assets/img/logo-laos.png') }}" alt="Logo"
            ...
            <div class="hidden md:flex items-center space-x-8">
                @foreach ($menus as $menu)
                    <a href="{{ url($menu['url']) }}"
                        class="text-gray-700 dark:text-gray-300 hover:text-green-500 dark:hover:text-green-400 font-medium transition-colors {{ request()->is($menu['active'] ?? $menu['url']) ? 'text-green-500 dark:text-green-400' : '' }}">
                        {{ $menu['title'] }}
                    </a>
                @endforeach
                ...
                @auth
                    <a href="{{ route('course.dashboard.index') }}"
                        class="px-6 py-2.5 bg-green-500 dark:bg-green-600 text-white rounded-full hover:bg-green-600 dark:hover:bg-green-700 transition-colors font-medium text-center">
                        Dashboard
                    </a>
                @endauth

                @guest
                    <a href="{{ route('course.login') }}"
                        class="px-6 py-2.5 bg-green-500 dark:bg-green-600 text-white rounded-full hover:bg-green-600 dark:hover:bg-green-700 transition-colors font-medium text-center">
                        Masuk
                    </a>
                @endguest
                {{-- <a href="{{ route('course.register') }}"
                    class="px-6 py-2.5 bg-yellow-500 dark:bg-yellow-600 text-white rounded-full hover:bg-yellow-600 dark:hover:bg-yellow-700 transition-colors font-medium text-center">
                    Daftar
                </a> --}}
            </div>
        ...
        <div class="p-6 bg-white dark:bg-gray-900 transition-colors">
            <div class="flex justify-between items-center mb-8">
                <a href="#" class="flex items-center space-x-3">
                    <img src="{{ asset('laos-course/front/assets/img/logo-laos.png') }}" alt="Logo"
                        class="w-10 h-10">
                    <span class="text-xl font-bold text-gray-900 dark:text-white transition-colors">UKM LAOS</span>
                </a>
            ...
            <div class="flex flex-col space-y-6 bg-white dark:bg-gray-900 h-screen transition-colors">
                @foreach ($menus as $menu)
                    <a href="{{ $menu['url'] }}"
                        class="text-gray-700 dark:text-gray-300 hover:text-green-500 dark:hover:text-green-400 font-medium text-lg transition-colors {{ request()->is($menu['url']) ? 'text-green-500 dark:text-green-400' : '' }}">
                        {{ $menu['title'] }}
                    </a>
                @endforeach
                ...
                @auth
                    <a href="{{ route('course.dashboard.index') }}"
                        class="text-center mt-4 w-full py-3 bg-green-500 dark:bg-green-600 text-white rounded-full hover:bg-green-600 dark:hover:bg-green-700 transition-colors font-medium">
                        Dashboard
                    </a>
                @endauth
                @guest
                    ...
                    <a href="{{ route('course.register') }}"
                        class="text-center mt-4 w-full py-3 bg-yellow-400 dark:bg-yellow-500 text-white rounded-full hover:bg-yellow-500 dark:hover:bg-yellow-600 transition-colors font-medium">
                        Daftar
                    </a>
            ...
            async function fetchSearchResults(query) {
                try {
                    const response = await $.ajax({
                        url: `/course/search?query=${encodeURIComponent(query)}`,
                        type: 'GET',
                        dataType: 'json'
                    });
