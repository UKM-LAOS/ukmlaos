                    ...
                    @foreach ($menu['children'] as $child)
                        ...
                        @else
                            <a href="{{ isset($child['route']) ? route($child['route']) : '#' }}"
                                class="block px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-green-500 dark:hover:text-green-400 {{ isset($child['route']) && Request::routeIs(isset($child['active']) ? $child['active'] : $child['route']) ? 'text-green-500 dark:text-green-400' : '' }}">
                                {{ $child['title'] }}
                            </a>
                        @endif
                    @endforeach
        ...
        @else
            <div class="mt-2 ml-4 border-l-2 border-gray-200 dark:border-gray-700 pl-4 hidden"
                ...
                @foreach ($menu['children'] as $child)
                    ...
                    @else
                        <a href="{{ isset($child['route']) ? route($child['route']) : '#' }}"
                            class="block py-2 text-gray-700 dark:text-gray-300 hover:text-green-500 dark:hover:text-green-400 {{ isset($child['route']) && Request::routeIs(isset($child['active']) ? $child['active'] : $child['route']) ? 'text-green-500 dark:text-green-400' : '' }}">
                            {{ $child['title'] }}
                        </a>
                    @endif
                @endforeach
