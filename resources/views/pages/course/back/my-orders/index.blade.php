                ...
                <div
                    ...
                    <div class="flex items-center mb-2 md:mb-0">
                        <div class="flex-shrink-0 h-10 w-10">
                            <img class="h-10 w-10 rounded-md object-cover"
                                src="{{ $transaction->courseStudent->course->getFirstMediaUrl('course-thumbnail') }}"
                                alt="{{ $transaction->courseStudent->course->judul }}">
                        </div>
                        <div class="ml-4">
                            <div class="text-sm font-medium text-gray-900 dark:text-white">
                                {{ $transaction->courseStudent->course->judul }}
                            </div>
                        </div>
                        <div class="md:hidden ml-auto flex items-center">
                            <span
                                ...
                                {{ ucwords(str_replace('-', ' ', $transaction->courseStudent->course->kategori->value)) }}
                            </span>
                        </div>
                    </div>
                    <div class="flex justify-between md:block">
                        ...
                        <span
                            ...
                            {{ ucwords($transaction->courseStudent->course->tipe->value) }}
                        </span>
                    </div>
                    ...
                    <div class="hidden md:block">
                        <span
                            ...
                            {{ $transaction->status === StatusEnum::MENUNGGU ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-100' : '' }}
                            {{ $transaction->status === StatusEnum::SUKSES ? 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100' : '' }}
                            {{ $transaction->status === StatusEnum::GAGAL ? 'bg-red-100 text-red-800 dark:bg-red-800 dark:text-red-100' : '' }}   
                            ">
                            {{ ucwords(str_replace('-', ' ', $transaction->status->value)) }}
                        </span>
                    </div>
                    <div class="flex justify-end mt-2 md:mt-0">
                        @if ($transaction->status === StatusEnum::SUKSES)
                            <a href="{{ route('course.dashboard.my-courses.show', $transaction->courseStudent->course->slug) }}"
                                class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300 bg-blue-100 dark:bg-blue-900/30 px-3 py-1 rounded-md text-sm mr-2">
                                Lihat Kursus
                            </a>
                        @endif
                    </div>
